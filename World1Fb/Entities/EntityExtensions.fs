...
module rec EntityExt = 
    ...
    let GetEntitiesAtLocationWithComponent (entities:EntityManager) (componentID:byte) (excludeEntityID:uint32 option) (location:LocationDataInt) = 
        location
        ...
        |> Array.Parallel.choose (TryGetComponent entities componentID)
    ...
    let TryGet (entities:EntityManager) (entityID:uint32) =
        entityID 
        |> entities.EntityExists 
        |> TrueSomeFalseNone (entities.GetComponents entityID)

    let TryGetComponent (entities:EntityManager) (componentID:byte) (entityID:uint32) : Option<Component> = 
        entityID
        |> TryGet entities
        |> Option.bind (fun cts -> 
