...
module rec Entities = 
    ...
    let CopyEntity (entities:Entities) (oldeid:uint32) =
    ...
    let GetComponentForEntities (entities:Entities) (componentID:byte) (entityIDs:uint32[]) = 
        componentID
    ...
    let GetComponentIDs (entities:Entities) (eid:uint32) =
        eid 
        |> entities.GetComponents 
        |> Array.Parallel.map (fun ct -> ct.ComponentID)

    let GetEntitiesAtLocationWithComponent (entities:Entities) (componentID:byte) (excludeEntityID:uint32 option) (location:LocationDataInt) = 
        location
    ...
    let GetHistory_Components (entities:Entities) (round:uint32 option) (componentID:byte) = 
    ...
    let GetHistory_Entities (entities:Entities) (round:uint32 option) = 
        let e,_,_ = entities.GetHistory round
        e

    let GetHistory_Locations (entities:Entities) (round:uint32 option) (location:LocationDataInt) =
        let _,_,locations = entities.GetHistory round
        match locations.ContainsKey location with
        | false -> [||]
        | true -> locations.Item location

    let GetLocation (entities:Entities) (entityID:uint32) = 
        (entityID |> entities.GetComponent FormComponentID).ToForm.Location

    let TryGet (entities:Entities) (entityID:uint32) =
        entityID 
        |> entities.EntityExists 
        |> TrueSomeFalseNone (entities.GetComponents entityID)

    let TryGetComponent (entities:Entities) (componentID:byte) (entityID:uint32) : Option<Component> = 
        entityID
        |> TryGet entities
        |> Option.bind (fun cts -> 
