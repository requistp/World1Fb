...
type EntitySystem(description:string, isActive:bool, enm:EntityManager, evm:EventManager) =
    ...
    member private me.onCreateEntity round (CreateEntity cts:GameEventData) =
        let checkComponentForEvents (c:Component) =
            ...
            | x when x = ControllerComponentType -> evm.RaiseEvent (ComponentAdded_Controller (ToController c))
            | x when x = EatingComponentType -> evm.RaiseEvent (ComponentAdded_Eating (ToEating c))
            | x when x = PlantGrowthComponentType -> evm.RaiseEvent (ComponentAdded_PlantGrowth (ToPlantGrowth c))
            | _ -> ()
