...
type FoodSystem(description:string, isActive:bool, enm:EntityManager, evm:EventManager) =
    ...
    member private me.onAllEaten round (ge:GameEventTypes) =
        ...
        match (ToFood (e.EateeID |> enm.GetComponentByType None FoodComponentID).[0]).FoodType.KillOnAllEaten with
        | false -> Ok None
        | true -> 
    ...
    member private me.onRegrowth round (ge:GameEventTypes) =
        ...
        let tryRegrowFood (f:FoodComponent) = 
            let pg = ToPlantGrowth (e.EntityID|>enm.GetComponentByType None PlantGrowthComponentID).[0]
        ...
        match (e.EntityID|>EntityExt.TryGetComponent enm None FoodComponentID) with
        | None -> Ok None
