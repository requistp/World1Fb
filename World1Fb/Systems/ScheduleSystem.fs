...
type ScheduleSystem(game:Game, isActive:bool) =
    ...
    //member private this.onScheduleEvent (ge:EventData_Generic) =
    ...
    //    schedule.Get
    //    |> Array.filter (fun se -> enm.Exists se.EventData.EntityID)
    //    |> Array.Parallel.iter (fun se -> executeAndReschedule se)

    override this.Initialize = 
        //evm.RegisterListener "ScheduleSystem" ScheduleEvent this.onScheduleEvent
